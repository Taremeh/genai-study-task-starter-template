...
import { getDemoProducts } from "utils/demoUtils"
...
const meilisearchSetup = async (client: MeiliSearch) => {
  ...
  client.getIndex('products').then((index) => {
    ...
    // We should construct the index from the storefrontClient instead of the demo products...
    const products = (await getDemoProducts()).hits
